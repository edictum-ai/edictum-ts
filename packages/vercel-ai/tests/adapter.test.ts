...
function makeGuard(
  options: ConstructorParameters<typeof Edictum>[0] = {},
): Edictum {
...
describe("tool success detection", () => {
  ...
  it("uses custom successCheck when provided", async () => {
    ...
    const guard = makeGuard({
      auditSink: sink,
      successCheck: (_name: string, result: unknown) =>
        result === "custom-ok",
    });
    const adapter = new VercelAIAdapter(guard);
...
describe("setPrincipal", () => {
  it("updates principal for subsequent calls", async () => {
    ...
    expect(
      (secondEvent?.principal as Record<string, unknown>)?.["userId"],
    ).toBe("user-123");
  });
});
...
describe("principalResolver", () => {
  it("resolver overrides static principal", async () => {
    ...
    const adapter = new VercelAIAdapter(guard, {
      ...
      principalResolver: (toolName: string) =>
        createPrincipal({ userId: `resolved-${toolName}` }),
    });

    await adapter._pre("SpecialTool", {}, "call-1");
    const event = sink.filter(AuditAction.CALL_ALLOWED)[0];
    expect(
      (event?.principal as Record<string, unknown>)?.["userId"],
    ).toBe("resolved-SpecialTool");
  });
});
