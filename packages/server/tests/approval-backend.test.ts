...
describe('ServerApprovalBackend pollInterval validation', () => {
  it('rejects pollInterval of 0', () => {
    const client = mockClient()
    expect(() => new ServerApprovalBackend(client, { pollInterval: 0 })).toThrow(EdictumConfigError)
  })
...
describe('ServerApprovalBackend timeout validation', () => {
  it('rejects timeout of 0', async () => {
    ...
    await expect(backend.requestApproval('Tool', {}, 'msg', { timeout: 0 })).rejects.toThrow(
      EdictumConfigError,
    )
  })
  ...
  it('rejects negative timeout', async () => {
    ...
    await expect(backend.requestApproval('Tool', {}, 'msg', { timeout: -10 })).rejects.toThrow(
      EdictumConfigError,
    )
  })
  ...
  it('rejects NaN timeout', async () => {
    ...
    await expect(backend.requestApproval('Tool', {}, 'msg', { timeout: NaN })).rejects.toThrow(
      EdictumConfigError,
    )
  })
  ...
  it('rejects Infinity timeout', async () => {
    ...
    await expect(backend.requestApproval('Tool', {}, 'msg', { timeout: Infinity })).rejects.toThrow(
      EdictumConfigError,
    )
  })
...
describe('ServerApprovalBackend pending map cap', () => {
  it('throws when MAX_PENDING is exceeded', async () => {
    ...
    await expect(backend.requestApproval('Tool', {}, 'msg')).rejects.toThrow(EdictumConfigError)
    await expect(backend.requestApproval('Tool', {}, 'msg')).rejects.toThrow(
  ...
  it('allows requests when under MAX_PENDING', async () => {
    ...
    await expect(backend.requestApproval('Tool', {}, 'msg')).resolves.toBeDefined()
  })
})
...
describe('ServerApprovalBackend toolName validation', () => {
  it('rejects empty toolName', async () => {
    ...
    await expect(backend.requestApproval('', {}, 'msg')).rejects.toThrow(EdictumConfigError)
  })
  ...
  it('rejects toolName with spaces', async () => {
    ...
    await expect(backend.requestApproval('bad tool', {}, 'msg')).rejects.toThrow(/Invalid toolName/)
  })
...
describe('ServerApprovalBackend message validation', () => {
  it('rejects empty message', async () => {
    ...
    await expect(backend.requestApproval('Bash', {}, '')).rejects.toThrow(EdictumConfigError)
  })
  ...
  it('accepts message of exactly 4096 chars', async () => {
    ...
    await expect(backend.requestApproval('Bash', {}, 'x'.repeat(4096))).resolves.toBeDefined()
  })
  ...
  it('rejects message exceeding 4096 chars', async () => {
    ...
    await expect(backend.requestApproval('Bash', {}, 'x'.repeat(4097))).rejects.toThrow(
      /message too long/,
    )
  })
...
describe('security', () => {
  it('rejects path traversal in requestApproval toolName', async () => {
    const client = mockClient()
    const backend = new ServerApprovalBackend(client)
    await expect(backend.requestApproval('../../escape', {}, 'msg')).rejects.toThrow(
      /Invalid toolName/,
    )
  })
  ...
  it('rejects null byte in requestApproval toolName', async () => {
    const client = mockClient()
    const backend = new ServerApprovalBackend(client)
    await expect(backend.requestApproval('tool\x00evil', {}, 'msg')).rejects.toThrow(
      /Invalid toolName/,
    )
  })
  ...
  it('rejects null byte in requestApproval message', async () => {
    const client = mockClient()
    const backend = new ServerApprovalBackend(client)
    await expect(backend.requestApproval('Bash', {}, 'msg\x00evil')).rejects.toThrow(
      /control characters/,
    )
  })
  ...
  it('rejects carriage return in requestApproval message', async () => {
    const client = mockClient()
    const backend = new ServerApprovalBackend(client)
    await expect(backend.requestApproval('Bash', {}, 'msg\x0devil')).rejects.toThrow(
      /control characters/,
    )
  })
  ...
  it('rejects vertical tab in requestApproval message', async () => {
    const client = mockClient()
    const backend = new ServerApprovalBackend(client)
    await expect(backend.requestApproval('Bash', {}, 'msg\x0bevil')).rejects.toThrow(
      /control characters/,
    )
  })
  ...
  it('rejects DEL character in requestApproval message', async () => {
    const client = mockClient()
    const backend = new ServerApprovalBackend(client)
    await expect(backend.requestApproval('Bash', {}, 'msg\x7fevil')).rejects.toThrow(
      /control characters/,
    )
  })
