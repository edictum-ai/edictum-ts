...
import { BundleVerificationError, verifyBundleSignature } from '../src/verification.js'
...
describe('empty input validation', () => {
  it('throws on empty signature', () => {
    ...
    expect(() => verifyBundleSignature(yamlBytes, '', publicKeyHex)).toThrow('Signature is empty')
  })

  it('throws on empty public key', () => {
    const yamlBytes = Buffer.from('data')
    expect(() => verifyBundleSignature(yamlBytes, 'AAAA', '')).toThrow('Public key is empty')
  })
})
...
describe('encoding validation', () => {
  ...
  it('throws on wrong-length public key', () => {
    ...
    expect(() => verifyBundleSignature(yamlBytes, sig, shortKey)).toThrow(BundleVerificationError)
  })
})
