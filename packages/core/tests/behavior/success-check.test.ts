...
describe('DefaultSuccessCheckStrings', () => {
  ...
  test('error_colon_prefix_returns_false', () => {
    expect(defaultSuccessCheck('TestTool', 'error: something broke')).toBe(false)
  })
  ...
  test('string_containing_error_not_at_start_returns_true', () => {
    expect(defaultSuccessCheck('TestTool', 'no error here')).toBe(true)
  })

  test('string_containing_fatal_not_at_start_returns_true', () => {
    expect(defaultSuccessCheck('TestTool', 'not fatal here')).toBe(true)
  })
...
describe('DefaultSuccessCheckObjects', () => {
  test('dict_with_is_error_true_returns_false', () => {
    expect(defaultSuccessCheck('TestTool', { is_error: true, message: 'fail' })).toBe(false)
  })

  test('dict_with_is_error_false_returns_true', () => {
    expect(defaultSuccessCheck('TestTool', { is_error: false, data: 'ok' })).toBe(true)
  })

  test('dict_without_is_error_returns_true', () => {
    expect(defaultSuccessCheck('TestTool', { status: 200, data: 'ok' })).toBe(true)
  })

  test('dict_with_is_error_truthy_string_returns_false', () => {
    // is_error is truthy (non-empty string)
    expect(defaultSuccessCheck('TestTool', { is_error: 'yes' })).toBe(false)
  })

  test('dict_with_is_error_zero_returns_true', () => {
    // is_error: 0 is falsy
    expect(defaultSuccessCheck('TestTool', { is_error: 0 })).toBe(true)
  })

  test('dict_with_is_error_null_returns_true', () => {
    // is_error: null is falsy
    expect(defaultSuccessCheck('TestTool', { is_error: null })).toBe(true)
  })

  test('dict_with_is_error_undefined_returns_true', () => {
    // is_error: undefined is falsy
    expect(defaultSuccessCheck('TestTool', { is_error: undefined })).toBe(true)
  })
})
