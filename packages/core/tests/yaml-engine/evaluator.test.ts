...
describe('SelectorResolution', () => {
  ...
  test('principal claims', () => {
    const env = _envelope(
      ...
      {},
      'production',
      createPrincipal({ claims: { department: 'platform' } }),
    )
    expect(evaluateExpression({ 'principal.claims.department': { equals: 'platform' } }, env)).toBe(
      true,
    )
  })
  ...
  test('output.text present', () => {
    const env = _envelope()
    expect(
      evaluateExpression({ 'output.text': { contains: 'secret' } }, env, 'this has a secret in it'),
    ).toBe(true)
  })
...
describe('StringOperators', () => {
  ...
  test('contains_any', () => {
    ...
    expect(evaluateExpression({ 'args.path': { contains_any: ['.env', '.secret'] } }, env)).toBe(
      true,
    )
  })
  ...
  test('matches', () => {
    ...
    expect(
      evaluateExpression({ 'args.command': { matches: '\\brm\\s+(-rf?|--recursive)\\b' } }, env),
    ).toBe(true)
  })
  ...
  test('matches_any', () => {
    ...
    expect(
      evaluateExpression({ 'args.command': { matches_any: ['\\brm\\b', '\\bmkfs\\b'] } }, env),
    ).toBe(true)
  })
...
describe('TypeMismatch', () => {
  ...
  test('starts_with on number', () => {
    const env = _envelope('read_file', { count: 42 })
    expect(evaluateExpression({ 'args.count': { starts_with: '4' } }, env)).toBeInstanceOf(
      PolicyError,
    )
  })
  ...
  test('matches on number', () => {
    const env = _envelope('read_file', { count: 42 })
    expect(evaluateExpression({ 'args.count': { matches: '\\d+' } }, env)).toBeInstanceOf(
      PolicyError,
    )
  })
})
...
describe('BooleanComposition', () => {
  test('all true', () => {
    ...
    const expr = {
      all: [{ 'tool.name': { equals: 'deploy' } }, { environment: { equals: 'production' } }],
    }
  ...
  test('not true', () => {
    const env = _envelope('read_file', {}, 'production', createPrincipal({ role: 'junior' }))
    expect(evaluateExpression({ not: { 'principal.role': { in: ['admin', 'sre'] } } }, env)).toBe(
      true,
    )
  })
  ...
  test('policy error propagates through all', () => {
    ...
    expect(evaluateExpression({ all: [{ 'args.count': { gt: 5 } }] }, env)).toBeInstanceOf(
      PolicyError,
    )
  })
  ...
  test('policy error propagates through any', () => {
    const env = _envelope('read_file', { count: 'not_a_number' })
    expect(evaluateExpression({ any: [{ 'args.count': { gt: 5 } }] }, env)).toBeInstanceOf(
      PolicyError,
    )
  })
  ...
  test('policy error propagates through not', () => {
    const env = _envelope('read_file', { count: 'not_a_number' })
    expect(evaluateExpression({ not: { 'args.count': { gt: 5 } } }, env)).toBeInstanceOf(
      PolicyError,
    )
  })
})
...
describe('EnvSelector', () => {
  ...
  test('env unset evaluates false', () => {
    ...
    expect(
      evaluateExpression({ 'env.NONEXISTENT_VAR_TEST': { equals: 'anything' } }, _envelope()),
    ).toBe(false)
  })
...
describe('CustomOperators', () => {
  test('custom operator is invoked', () => {
    ...
    const result = evaluateExpression({ 'args.count': { is_even: true } }, env, null, {
...
describe('CustomSelectors', () => {
  test('custom selector is resolved', () => {
    ...
    const result = evaluateExpression({ 'custom.level': { equals: 'high' } }, env, null, {
...
describe('security', () => {
  ...
  test('null byte in selector path returns false (not crash)', () => {
    ...
    const result = evaluateExpression({ 'args.path\x00evil': { equals: 'data' } }, env)
  ...
  test('prototype pollution attempt via __proto__ selector', () => {
    ...
    const result = evaluateExpression({ 'args.__proto__.admin': { equals: true } }, env)
